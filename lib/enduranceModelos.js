...
import { calcularVDOT, calcularFTP, calcularFTPRampa, nivelPorVDOT, nivelPorFTP } from './enduranceZonas.js';
...
function volumeLongaoDaSemana(modalidade, chaveAlvo, chaveFase, semRest, ehDeload, nivel = 'intermediario') {
  ...
  // Escala os alvos pelo nível do aluno (iniciante recebe fração do avançado).
  ...
  let v;
  if (chaveFase === 'base') {
    v = alvoMin;                                      // base: longão estável e modesto
  } else {
...
function prioridadeTipo(tipo) {
  ...
  if (['intervalado','tempo','limiar','vo2','fartlek','sweetspot','tiros',
       'subidas','forca','sprint'].includes(tipo)) return 2;              // qualidade
  if (['rodagem','endurance'].includes(tipo)) return 3;                    // contínuo/base
  return 4;                                                                // regenerativo/educativo (corta 1º)
}

/**
 * Ajusta a semana pra ter EXATAMENTE `freq` dias de treino, preservando as
 * posições do modelo (bom espaçamento) e cortando por prioridade — o
 * regenerativo/fácil sai primeiro; longão e qualidade ficam. Se já tem ≤ freq
 * dias, não mexe.
 */
export function aplicarFrequencia(semana, freq) {
  if (!freq) return semana;
  const treinos = semana.map((d, i) => ({ i, tipo: d.rest ? null : d.tipo })).filter(x => x.tipo);
  if (treinos.length <= freq) return semana;
  const nCortar = treinos.length - freq;
  ...
  return semana.map((d, i) => (cortar.has(i) ? { rest: true } : d));
}
...
export function semanaSugerida(modalidade, chaveFase, distanciaProva = null, semRest = null, nivel = 'intermediario', freq = null, semanaIdx = 0) {
  ...
  if (!progAtiva) {
    ...
    semana = semana.map(d => {
      if (d.rest) return d;
      // LONGÃO — escala a distância pelo nível (mesma regra do modo com prova):
      // iniciante não aguenta o longão fixo do modelo.
      if (d.tipo === 'longao' && fNivel !== 1) {
    ...
    return aplicarFrequencia(semana, freq);
  }
