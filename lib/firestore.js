...
export async function sincronizarProgramaMuscular(aluno) {
  ...
  // Relê antes de mexer em qualquer coisa — se outra aba/dispositivo já
  // avançou o mês primeiro, não faz nada.
  const snapAtual = await getDoc(doc(db, 'alunos', aluno.id));
  const mesAgora = snapAtual.data()?.programaMuscular?.mesAtual ?? meta.mesAtual;
  if (mesAgora !== meta.mesAtual) return mesAgora;
