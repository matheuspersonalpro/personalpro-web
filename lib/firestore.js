...
export async function atribuirProgramaMuscular(aluno, programaId, mesInicial = 1) {
  ...
  // Salva o metadado no cadastro do aluno — SEM isso (bug real, achado hoje),
  // os treinos eram criados certinho mas o app nunca sabia "esse aluno está
  // no programa X, mês Y", então o avanço automático de mês nunca rodava e
  // o botão de remover programa nunca aparecia (dependem desse campo).
  // dataInicio ancorado igual ao app mobile, pro mês inicial bater com a
  // regra dos 30 dias.
  const ini = new Date();
  ini.setDate(ini.getDate() - (mesInicial - 1) * 30);
  const dataInicio = ini.toISOString().slice(0, 10);
  await updateDoc(doc(db, 'alunos', aluno.id), {
    programaMuscular: { programaId, mesAtual: mesInicial, dataInicio },
  });
}
...
// Sincroniza com o calendário: se já passaram 30 dias, avança o mês e
// regenera os treinos. Idempotente — chamar ao abrir a ficha do aluno.
// Existia SÓ no app mobile — o site nunca tinha isso, então um aluno cujo
// personal só usa o site ficava travado pra sempre no mês em que o
// programa foi atribuído, mesmo com o tempo passando. Mesma lógica atômica
// (writeBatch) e checagem otimista do app mobile — ver trocarMesProgramaAtomico
// em C:\Users\mathe\PersonalPro\firestore.js pro comentário completo.
export async function sincronizarProgramaMuscular(aluno) {
  const { mesAlvoPrograma, gerarProgramaMes, PROGRAMAS } = await import('./programaMusculacao.js');
  // SEMPRE relê do Firestore antes de decidir — nunca confia no
  // `aluno.programaMuscular` recebido por parâmetro (mesmo bug e mesmo fix
  // do app mobile: um mesAtual local novo combinado com uma dataInicio local
  // desatualizada fazia essa função REGREDIR o mês sozinha na próxima
  // sincronização — ver o comentário completo em
  // C:\Users\mathe\PersonalPro\firestore.js).
  ...
  const alvo = mesAlvoPrograma(meta.dataInicio);
  if (alvo === meta.mesAtual) return meta.mesAtual;
  ...
  const ocultos = await buscarExerciciosOcultos();
  const novos = gerarProgramaMes(meta.programaId, alvo, ocultos);
  const categoria = PROGRAMAS[meta.programaId]?.nome || 'Programa';

  // writeBatch atômico (tudo-ou-nada) — apaga os treinos do mês anterior,
  // cria os do mês novo e atualiza o metadado, tudo num único commit.
  const lote = writeBatch(db);
  antigos.forEach(d => lote.delete(d.ref));
  novos.forEach(t => lote.set(doc(collection(db, 'treinos')), {
    nome: t.nome, foco: t.foco, dificuldade: t.dificuldade,
    exercicios: t.exercicios || [],
    categoria, orientacoes: t.orientacoes || '',
    alunoId: aluno.id, alunoNome: aluno.nome || '',
    template: false, origem: 'programa',
    programaId: meta.programaId, programaMes: alvo,
    personalId: pid, criadoEm: serverTimestamp(),
  }));
  lote.update(doc(db, 'alunos', aluno.id), { 'programaMuscular.mesAtual': alvo });
  await lote.commit();
  return alvo;
}
