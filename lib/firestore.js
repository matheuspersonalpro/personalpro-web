...
export async function atribuirProgramaMuscular(aluno, programaId, mesInicial = 1) {
  ...
  const pid = personalId();
  // Apaga treinos de programa existentes do aluno
  const snap = await getDocs(query(collection(db, 'treinos'), where('alunoId', '==', aluno.id)));
  ...
  // Gera e salva treinos do mês inicial
  const treinos = gerarProgramaMes(programaId, mesInicial, new Set());
